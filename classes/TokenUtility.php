...
class TokenUtility {
   ...
   * @var int
   */
  protected static $padded_order_id_length = 8;
   ...
   * @throws \Exception
   */
  public static function get_url_token($order, $type_key) {
    if (!$order['token']) {
      throw new \Exception('Tried to generate a URL token for an order without record token. This might be a security risk.', 1394637804);
    }
    if (!$type_key) {
    ...
    return str_pad($order['uid'], self::$padded_order_id_length, '0', STR_PAD_LEFT) . hash('sha512', $order['token'] . $type_key);
  }
   ...
   * @return bool
   */
  public static function evaluate_url_token($order, $token, $type_key) {
    $actual_token = self::get_url_token($order, $type_key);
